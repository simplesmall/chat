...
const msgHandle = {
  ...
  pushMsg (msgs, root) {
    ...
    let dataMsgs = root.$store.getters.msgsGetter || {}
    dataMsgs = root.$store.getters.nimInstanceGetter.mergeMsgs(dataMsgs, msgs)
    let msgObj = {'key': sessionId, 'val': dataMsgs}
    root.$store.dispatch('setMsgsAct', msgObj)
    // data.msgs = data.msgs || {}
